...
export function ArticlesManager() {
  ...
  if (editing) {
    return <ArticleEditor article={editing === 'new' ? null : editing} onDone={() => setEditing(null)} />;
  }
      ...
      {articles.map((a) => (
        <div key={a.id} style={{ ...articleRowStyle, opacity: a.published ? 1 : 0.55 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {a.subtitle && <div style={articleSubStyle}>{a.subtitle}</div>}
            <div style={articleDateStyle}>{a.date.toLocaleDateString('ja-JP')}</div>
          </div>
          <div style={articleActionsStyle}>
            <button type="button" onClick={() => handleTogglePublish(a)} style={iconBtn} title={a.published ? '非公開' : '公開'}>
              {a.published ? <IoEye size={16} color="#5a9e6e" /> : <IoEyeOff size={16} color={colors.textSecondary} />}
            </button>
            <button type="button" onClick={() => handleTogglePin(a)} style={iconBtn} title={a.stable ? '固定解除' : '固定'}>
...
/* ── Article Editor (simplified: title + summary, no rich text) ── */
function ArticleEditor({ article, onDone }: { article: ArticleDoc | null; onDone: () => void }) {
  ...
  const canSubmit = title.trim() && (descriptions.trim() || externalLink.trim());
    ...
    <div style={editorWrapStyle}>
      <form onSubmit={handleSave} style={editorFormStyle}>
        <div style={editorHeaderStyle}>
          <h3 style={editorHeadingStyle}>{article ? '記事を編集' : '新規作成'}</h3>
          <button type="button" onClick={onDone} style={editorCancelStyle}>キャンセル</button>
        </div>

        <label style={fieldLabelStyle}>
          タイトル *
          <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required style={fieldInputStyle} placeholder="お知らせのタイトル" />
        </label>

        <label style={fieldLabelStyle}>
          サブタイトル
          <input type="text" value={subtitle} onChange={(e) => setSubtitle(e.target.value)} style={fieldInputStyle} placeholder="補足テキスト（任意）" />
        </label>
        ...
        <button type="submit" disabled={busy || uploading || !canSubmit} style={saveBtnStyle}>
      ...
      <div style={previewPanelStyle}>
        ...
        <div style={previewCardStyle}>
          ...
          <div style={previewMetaStyle}>
            {stable && <span style={previewPinBadgeStyle}><IoPin size={11} color="#fff" /> 固定</span>}
            <span style={previewDateStyle}>{date}</span>
            {!published && <span style={previewDraftStyle}>下書き</span>}
            {externalLink && <span style={previewLinkTagStyle}><IoLinkOutline size={11} /> 外部リンク</span>}
          </div>
...
const articleRowStyle: React.CSSProperties = {
  ...
  background: 'rgba(255,255,255,0.6)', border: '1px solid rgba(255,255,255,0.7)', boxShadow: '2px 2px 6px rgba(174,164,204,0.1)',
};
const rowThumbStyle: React.CSSProperties = { width: 48, height: 48, borderRadius: 8, objectFit: 'cover', flexShrink: 0 };
const articleTitleRowStyle: React.CSSProperties = { fontSize: 14, fontWeight: 600, color: colors.textPrimary, display: 'flex', alignItems: 'center', gap: 4 };
const articleTitleTextStyle: React.CSSProperties = { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' };
const draftTagStyle: React.CSSProperties = { fontSize: 9, fontWeight: 600, padding: '1px 6px', borderRadius: 4, background: 'rgba(180,180,180,0.2)', color: colors.textSecondary, flexShrink: 0 };
const articleSubStyle: React.CSSProperties = { fontSize: 11, color: colors.textSecondary, marginTop: 1 };
const articleDateStyle: React.CSSProperties = { fontSize: 11, color: colors.textSecondary, marginTop: 2 };
const articleActionsStyle: React.CSSProperties = { display: 'flex', gap: 4, flexShrink: 0 };
...
const editorWrapStyle: React.CSSProperties = { display: 'flex', gap: 20, alignItems: 'flex-start' };
const editorFormStyle: React.CSSProperties = {
  flex: '1 1 50%', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 14, padding: '20px 22px', borderRadius: 16,
  background: 'rgba(255,255,255,0.65)', border: '1px solid rgba(255,255,255,0.7)', boxShadow: '3px 3px 10px rgba(174,164,204,0.12)',
};
const editorHeaderStyle: React.CSSProperties = { display: 'flex', justifyContent: 'space-between', alignItems: 'center' };
const editorHeadingStyle: React.CSSProperties = { fontSize: 16, fontWeight: 700, color: colors.textPrimary, margin: 0 };
const editorCancelStyle: React.CSSProperties = { background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: colors.textSecondary, fontWeight: 500 };
...
const removeThumbBtnStyle: React.CSSProperties = { background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: '#c25a65', fontWeight: 500, padding: '4px 0' };
const checkRowStyle: React.CSSProperties = { display: 'flex', gap: 20, flexWrap: 'wrap' };
const checkLabelStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 500, color: colors.textPrimary, cursor: 'pointer' };
const saveBtnStyle: React.CSSProperties = { padding: '11px 0', borderRadius: 10, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg, #a388c8, ${colors.primary})`, color: '#fff', fontSize: 13, fontWeight: 600, boxShadow: '0 3px 10px rgba(145,120,189,0.3)' };

const previewPanelStyle: React.CSSProperties = { flex: '1 1 50%', minWidth: 0, position: 'sticky', top: 24 };
const previewLabelStyle: React.CSSProperties = { fontSize: 11, fontWeight: 600, color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 };
const previewCardStyle: React.CSSProperties = { borderRadius: 16, padding: '22px 24px', background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(255,255,255,0.7)', boxShadow: '3px 3px 10px rgba(174,164,204,0.12), -2px -2px 6px rgba(255,255,255,0.7)', maxHeight: '75vh', overflowY: 'auto' };
const previewThumbStyle: React.CSSProperties = { width: '100%', maxHeight: 200, objectFit: 'cover', borderRadius: 10, marginBottom: 14 };
const previewMetaStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' };
const previewPinBadgeStyle: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 9px', borderRadius: 10, background: 'linear-gradient(135deg, #FFD54A, #FFB33C)', color: '#fff', fontSize: 10, fontWeight: 700 };
const previewDateStyle: React.CSSProperties = { fontSize: 11, color: colors.textSecondary };
const previewDraftStyle: React.CSSProperties = { fontSize: 9, fontWeight: 600, padding: '1px 6px', borderRadius: 4, background: 'rgba(180,180,180,0.2)', color: colors.textSecondary };
