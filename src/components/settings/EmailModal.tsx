...
import { changeEmail, readableAuthError } from '@/services/auth';
...
export function EmailModal({ visible, onClose }: EmailModalProps) {
  const { user } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      await changeEmail(password, trimmed);
      setDone(true);
    } catch (err) {
      ...
      <Pressable style={styles.overlay} onPress={handleClose}>
        ...
        >
          <Pressable style={styles.card} onPress={() => undefined}>
            ...
            <Text style={styles.sub}>現在: {user?.email || '(未設定)'}</Text>

            {done ? (
              <View style={styles.successBox}>
                <Ionicons name="checkmark-circle" size={20} color="#5a9e6e" />
                <Text style={styles.successText}>メールアドレスを変更しました。</Text>
              </View>
            ) : (
