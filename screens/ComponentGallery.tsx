/**
 * ComponentGallery.tsx — FLUX RING Skia 参照実装デモ（旧 App.tsx）
 * 4つの部品（ArtworkCard / Hero / StarBurst / PurchaseTransition）を
 * 上部タブで切り替えて、実機で目視確認するためのデモ画面。
 *
 * 注意: これは確認用デモ。本アプリのナビゲーションは App.tsx 参照。
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.stage}>
        {tab === 'card' && <ArtworkCard width={cardW} imageUri={DEMO_IMG} />}
        ...
        {tab === 'stars' && (
          <>
            <StarBurst
              ...
              edgeRadius={cardW / 2}
            />
            <Pressable style={styles.btn} onPress={() => starRef.current?.trigger()}>
...
const styles = StyleSheet.create({
  ...
  tabs: { flexDirection: 'row', paddingTop: 56, paddingHorizontal: 12, gap: 8 },
  tab: {
    paddingVertical: 8,
