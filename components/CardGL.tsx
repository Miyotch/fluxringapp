...
// ±22° に制限する（どの方向にドラッグしても刻印が読める角度で止まる）。
const CARD_ANG_CLAMP = 0.384; // 22° = 22*π/180 ≒ 0.384 rad
// 重力オートリターン: 手を離すと弱いバネで「いま見えている面」の定位置へ戻す。
// 残差は exp(-K*T)。95%戻るまでの時間 T ≒ ln(20)/K なので K=2.0 で約1.5秒
// （main で確定した値をそのまま使う）。
...
export type CardGLProps = {
  ...
  /**
   * 裏面（フリップ後）の表示倍率を明示指定する。未指定なら frame から
   * 参照どおり算出する（computeBackScale）。PlayerScreen は「裏面の絶対サイズを
   * ホームと揃える」ために自前で逆算した値を渡すので、その上書き用。
   */
  backScale?: number;
