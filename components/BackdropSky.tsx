 ...
 *
 * 待機中の横流れについて（2026-09-14 / 09-22 改訂）:
 *   ホームではカードを固定したまま、星（parallaxX）だけを左へ流し続ける。星は元から
 *   周期 W のタイル。天の川は流さず、雲がその場でそよぐだけ（0.1.0 (100) と同じ）。
 *   09-14〜09-21 は天の川も雲と星520を 3 組ずつ置いて流していたが、発熱の再発で戻した。
...
export type BackdropSkyProps = {
   ...
   * 確実に効くほうを採る。そのぶん星が流れている間は、星の Canvas 2 枚が背景の
   * 時計と同じ 30fps で塗り直される（裏返し中・アプリ背面では止まる）。
   */
   ...
   * 大きさのまま。雲のそよぎの**外側**に掛けるので、そよぐ中心点ごと
...
const BackdropSkyImpl: React.FC<BackdropSkyProps> = ({
  ...
  const tree = useMemo(
    ...
    [W, H, scale, clouds, nebStarGroups, clock, stop, nebAdjust],
  );
